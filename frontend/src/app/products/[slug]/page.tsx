...
export default async function ProductPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main style={{ background: 'var(--color-bg)' }}>
          ...
          <div style={{
              ...
              <div style={{
                ...
                {product.badge && (
                  <span style={{
                    ...
                    fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '.05em',
                ...
                {discount && (
                  <span style={{
                    ...
                    padding: '.3rem .75rem', borderRadius: 'var(--radius-full)',
                  }}>−{discount}%</span>
                )}
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
              ...
              <div>
                ...
                {product.subtitle && (
                  <p style={{ fontSize: 'var(--text-base)', color: 'var(--color-text-muted)', marginTop: 'var(--space-3)', lineHeight: 1.6 }}>
                    {product.subtitle}
                  </p>
                )}
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 'var(--space-4)', flexWrap: 'wrap' }}>
                ...
                }}>{formatAUD(product.price)}</span>
                {origPrice && (
                  <s style={{ fontSize: 'var(--text-base)', color: 'var(--color-text-faint)' }}>{formatAUD(origPrice)}</s>
                )}
                {discount && (
                  <span style={{
              ...
              <div style={{
                ...
                {[
                  ['🚚', 'Free Shipping', 'Orders over A$150'],
                  ['↩️', 'Easy Returns', '15-day hassle-free'],
                  ['✅', '100% Authentic', 'Direct from artisans'],
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                {[
                    ...
                    content: (
                      <>
                        <p style={{ color: 'var(--color-text-muted)', lineHeight: 1.7, marginBottom: 'var(--space-4)' }}>
                          {description ?? 'Handcrafted by skilled artisans using traditional techniques. Each piece is unique and may have slight variations that are part of its handmade character.'}
                        </p>
                  ...
                  },
                  {
                    ...
                    content: <p style={{ color: 'var(--color-text-muted)', lineHeight: 1.7 }}>Free standard shipping on orders over A$150. Express delivery available at checkout. Delivered Australia-wide within 5–9 business days. Returns accepted within 15 days of delivery in original, unworn condition.</p>,
                  },
                ].map(({ title, content }) => (
                  <details key={title} style={{
                    background: 'var(--color-surface)',
                    border: '1px solid var(--color-divider)',
      ...
      {/* Decorative band */}
      <div style={{
        background: 'linear-gradient(135deg, var(--color-primary) 0%, #8b2f54 100%)',
          ...
          <div className="container">
            <div style={{ textAlign: 'center', marginBottom: 'var(--space-10)' }}>
              ...
              <h2 style={{
                ...
                marginTop: 'var(--space-4)', color: 'var(--color-text)',
              }}>More from {categoryLabel[product.category]}</h2>
            </div>
