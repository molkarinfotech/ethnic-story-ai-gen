  ...
  if (done) return (
    <main style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--color-bg)', padding: '2rem' }}>
      <div style={{ textAlign: 'center', maxWidth: '420px' }}>
        ...
        <div style={{ marginTop: '2rem', display: 'flex', flexDirection: 'column', gap: '.75rem', alignItems: 'center' }}>
          <a href="/login" style={{ background: 'var(--color-primary)', color: 'white', padding: '.75rem 2rem', borderRadius: 'var(--radius-full)', textDecoration: 'none', fontWeight: 700, fontSize: 'var(--text-sm)' }}>Back to login</a>
          <a href="/" style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-muted)' }}>Continue browsing</a>
        </div>
      </div>
    </main>
  );
    ...
    <main style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--color-bg)', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.5rem,1.2rem+1vw,2rem)', color: 'var(--color-text)', margin: 0 }}>Create an account</h1>
          <p style={{ color: 'var(--color-text-muted)', marginTop: '.5rem', fontSize: 'var(--text-sm)' }}>Join Ethnic Story for order history &amp; wishlists</p>
        </div>
          ...
          <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <Field label="Full name">
              <input type="text" value={name} onChange={e => setName(e.target.value)} required
                style={inputStyle} placeholder="Your full name" autoComplete="name" />
            </Field>
            ...
            <Field label="Password">
              ...
              <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-faint)', marginTop: '.35rem' }}>Minimum 6 characters</p>
            </Field>

            <button type="submit" disabled={loading}
              style={{ width: '100%', padding: '.85rem', background: 'var(--color-primary)', color: 'white', border: 'none', borderRadius: 'var(--radius-md)', fontWeight: 700, fontSize: 'var(--text-sm)', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? .7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '.5rem', marginTop: '.5rem' }}>
              {loading ? <><Spinner size={18} color="white" /> Creating account…</> : 'Create account with Email'}
            </button>
          </form>

          <p style={{ textAlign: 'center', marginTop: '1.25rem', fontSize: 'var(--text-xs)', color: 'var(--color-text-faint)' }}>
            🔒 Secure sign-up · Your data is never shared
          </p>
        </div>
