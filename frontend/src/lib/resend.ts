...
/** Confirmation email for Coming Soon launch notifications */
export function buildComingSoonSubscribedEmail(productName: string, email: string): EmailPayload {
  const content = `
    <h1 style="font-family:Georgia,serif;color:#1d4ed8;font-size:24px;margin:0 0 8px;">You\u2019re on the launch list! &#128640;</h1>
    <p style="color:#6b7280;margin:0 0 28px;font-size:15px;">We\u2019ll send you an email at <strong>${email}</strong> the moment <strong>${productName}</strong> is available to purchase.</p>
    <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:20px;text-align:center;margin-bottom:28px;">
      <span style="font-size:2rem;">&#128640;</span>
      <div style="font-size:14px;color:#1d4ed8;font-weight:600;margin-top:8px;">Launch notification set for</div>
      <div style="font-size:16px;font-weight:700;color:#1a1a1a;margin-top:4px;">${productName}</div>
      <div style="font-size:12px;color:#6b7280;margin-top:8px;">We\u2019ll only send one email when this item launches. No spam.</div>
    </div>
    <div style="text-align:center;">
      <a href="https://ethnicstory.com.au/collections" style="display:inline-block;padding:14px 32px;background:#1d4ed8;color:white;text-decoration:none;border-radius:50px;font-weight:700;font-size:14px;">Browse Our Collection</a>
    </div>`;
  return {
    to:      email,
    subject: `You\u2019re on the list for ${productName} &#128640; Ethnic Story`,
    html:    brandWrap(content),
  };
}
